...
def get_prescription_from_list(prescription_list, drug,  status):
    """
    Check's prescritions expiry status
    :param prescription_list:
    :param status:
    :return:
    """
    logger.info("info: The following is the prescritions lists : {}".format(
        prescription_list
    ))
    for prescription in prescription_list:
        if status == 'valid':
            if '2020' not in prescription["prescriptionExpirationDate"] and drug == prescription['drugNdc']:
                return prescription
        elif status == 'expired':
            if '2020' in prescription["prescriptionExpirationDate"] and drug == prescription['drugNdc']:
                return prescription
    logger.error("Error: Prescription not found")
    return False


def update_patient_address(patient_details, order_payload):
    """
    Add patient address details to the order payload
    :param patient_details:
    :param prescription_payload:
    :return:
    """
    if patient_details.get("firstName", None):
        order_payload["shipToFirstName"] = patient_details.get("firstName")
    if patient_details.get("lastName", None):
        order_payload["shipToLastName"] = patient_details.get("lastName")
    if patient_details.get("address1", None):
        order_payload["shipToAddress"] = patient_details.get("address1")
    if patient_details.get("address2", None):
        order_payload["shipToAddress2"] = patient_details.get("address2")
    if patient_details.get("zipCode", None):
        order_payload["shipToZipCode"] = patient_details.get("zipCode")
    return order_payload


def get_drug_price_from_ndc(list_of_drug, drug_ndc):
    """
    GEt Drug by entering drug NDC from list of drugs on a aite
    :param list_of_drug:
    :param drug_ndc:
    :return:
    """
    logger.info("info: The following is the drugs lists : {}".format(
        list_of_drug
    ))
    for drug in list_of_drug:
        if drug['ndc'] == drug_ndc:
            return drug
    logger.error("Error: Drug by NDC not found")
    return False
